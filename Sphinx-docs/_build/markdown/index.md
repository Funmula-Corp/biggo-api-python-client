<!-- biggo_api documentation master file, created by
sphinx-quickstart on Thu Mar  9 11:29:02 2023.
You can adapt this file completely to your liking, but it should at least
contain the root `toctree` directive. -->
# Welcome to biggo_api’s documentation!

# Contents:


* [biggo_api package](biggo_api.md)


    * [Subpackages](biggo_api.md#subpackages)


        * [biggo_api.async_clients package](biggo_api.async_clients.md)


            * [Submodules](biggo_api.async_clients.md#submodules)


            * [biggo_api.async_clients._auth module](biggo_api.async_clients.md#module-biggo_api.async_clients._auth)


            * [biggo_api.async_clients._base module](biggo_api.async_clients.md#module-biggo_api.async_clients._base)


            * [biggo_api.async_clients._user module](biggo_api.async_clients.md#module-biggo_api.async_clients._user)


            * [biggo_api.async_clients._video module](biggo_api.async_clients.md#module-biggo_api.async_clients._video)


            * [biggo_api.async_clients.api module](biggo_api.async_clients.md#module-biggo_api.async_clients.api)


            * [Module contents](biggo_api.async_clients.md#module-biggo_api.async_clients)


        * [biggo_api.clients package](biggo_api.clients.md)


            * [Submodules](biggo_api.clients.md#submodules)


            * [biggo_api.clients._auth module](biggo_api.clients.md#module-biggo_api.clients._auth)


            * [biggo_api.clients._base module](biggo_api.clients.md#module-biggo_api.clients._base)


            * [biggo_api.clients._user module](biggo_api.clients.md#module-biggo_api.clients._user)


            * [biggo_api.clients._video module](biggo_api.clients.md#module-biggo_api.clients._video)


            * [biggo_api.clients.api module](biggo_api.clients.md#module-biggo_api.clients.api)


            * [Module contents](biggo_api.clients.md#module-biggo_api.clients)


        * [biggo_api.data_models package](biggo_api.data_models.md)


            * [Submodules](biggo_api.data_models.md#submodules)


            * [biggo_api.data_models.product module](biggo_api.data_models.md#module-biggo_api.data_models.product)


            * [biggo_api.data_models.user module](biggo_api.data_models.md#module-biggo_api.data_models.user)


            * [biggo_api.data_models.video module](biggo_api.data_models.md#module-biggo_api.data_models.video)


            * [Module contents](biggo_api.data_models.md#module-biggo_api.data_models)


        * [biggo_api.enum package](biggo_api.enum.md)


            * [Submodules](biggo_api.enum.md#submodules)


            * [biggo_api.enum.access module](biggo_api.enum.md#module-biggo_api.enum.access)


            * [biggo_api.enum.process_status module](biggo_api.enum.md#module-biggo_api.enum.process_status)


            * [Module contents](biggo_api.enum.md#module-biggo_api.enum)


    * [Submodules](biggo_api.md#submodules)


    * [biggo_api.exception module](biggo_api.md#module-biggo_api.exception)


        * [`BigGoAPIError`](biggo_api.md#biggo_api.exception.BigGoAPIError)


            * [`BigGoAPIError.response`](biggo_api.md#biggo_api.exception.BigGoAPIError.response)


    * [biggo_api.responses module](biggo_api.md#module-biggo_api.responses)


        * [`BaseResponse`](biggo_api.md#biggo_api.responses.BaseResponse)


            * [`BaseResponse.result`](biggo_api.md#biggo_api.responses.BaseResponse.result)


        * [`Error`](biggo_api.md#biggo_api.responses.Error)


            * [`Error.code`](biggo_api.md#biggo_api.responses.Error.code)


            * [`Error.message`](biggo_api.md#biggo_api.responses.Error.message)


        * [`ErrorResponse`](biggo_api.md#biggo_api.responses.ErrorResponse)


            * [`ErrorResponse.error`](biggo_api.md#biggo_api.responses.ErrorResponse.error)


        * [`UserVideo`](biggo_api.md#biggo_api.responses.UserVideo)


            * [`UserVideo.data`](biggo_api.md#biggo_api.responses.UserVideo.data)


            * [`UserVideo.size`](biggo_api.md#biggo_api.responses.UserVideo.size)


        * [`UserVideoResponse`](biggo_api.md#biggo_api.responses.UserVideoResponse)


            * [`UserVideoResponse.user_video`](biggo_api.md#biggo_api.responses.UserVideoResponse.user_video)


        * [`VideoDeleteResponse`](biggo_api.md#biggo_api.responses.VideoDeleteResponse)


            * [`VideoDeleteResponse.result`](biggo_api.md#biggo_api.responses.VideoDeleteResponse.result)


        * [`VideoPermissionResponse`](biggo_api.md#biggo_api.responses.VideoPermissionResponse)


            * [`VideoPermissionResponse.at_userid`](biggo_api.md#biggo_api.responses.VideoPermissionResponse.at_userid)


            * [`VideoPermissionResponse.region`](biggo_api.md#biggo_api.responses.VideoPermissionResponse.region)


            * [`VideoPermissionResponse.userid`](biggo_api.md#biggo_api.responses.VideoPermissionResponse.userid)


        * [`VideoResponse`](biggo_api.md#biggo_api.responses.VideoResponse)


            * [`VideoResponse.size`](biggo_api.md#biggo_api.responses.VideoResponse.size)


            * [`VideoResponse.user`](biggo_api.md#biggo_api.responses.VideoResponse.user)


            * [`VideoResponse.video`](biggo_api.md#biggo_api.responses.VideoResponse.video)


        * [`VideoUpdateResponse`](biggo_api.md#biggo_api.responses.VideoUpdateResponse)


            * [`VideoUpdateResponse.data`](biggo_api.md#biggo_api.responses.VideoUpdateResponse.data)


        * [`VideoUploadResponse`](biggo_api.md#biggo_api.responses.VideoUploadResponse)


            * [`VideoUploadResponse.video_id`](biggo_api.md#biggo_api.responses.VideoUploadResponse.video_id)


    * [Module contents](biggo_api.md#module-biggo_api)


# Indices and tables


* [Index](genindex.md)


* [Module Index](py-modindex.md)


* [Search Page](search.md)
